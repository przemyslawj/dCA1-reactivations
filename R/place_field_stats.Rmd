...
Place field correlation from habituation to beforetest
```{r}
cell.cor.learning.change = function(animal_name, cell_name, 
                                    fst.dayname = 'habituation day#2',
                                    snd.dayname = 'habituation day#3') {
  fst.day = subset(trials.meta.df, animal == animal_name & day_desc == fst.dayname)$date[1] %>% as.character()
  fst.pf = create.partial.df(run.fields, run.occupancies, animal_name, fst.day, cell_name, 'fst')
  fields.list = run.fields
  occupancies.list = run.occupancies
  if (startsWith(snd.dayname, 'learning')) {
    fields.list = beforetest.fields
    occupancies.list = beforetest.occupancies
  }
  snd.day = subset(trials.meta.df, animal == animal_name & day_desc == snd.dayname)$date[1] %>% as.character()
  snd.pf = create.partial.df(fields.list, occupancies.list, animal_name, snd.day, cell_name, 'snd')
  if (nrow(fst.pf) == 0 || nrow(snd.pf) == 0) {
    return(NA)
  }
  x = field.cor(fst.pf, snd.pf, nbins, make.cor.plot=FALSE)

  return(x$cor)
}

habit.learning.pf.cor = bind_rows(
  run.trials.si %>%
  filter(day_desc == 'habituation day#2', signif.si) %>%
  dplyr::mutate(session.cor = map2_dbl(animal, cell_id, 
                                       ~ cell.cor.learning.change(as.character(.x), .y)),
                comparison='2_habit'),
  run.trials.si %>%
  filter(day_desc == 'habituation day#3', signif.si) %>%
  dplyr::mutate(session.cor = map2_dbl(animal, cell_id, 
                                       ~ cell.cor.learning.change(as.character(.x), .y,
                                                                  fst.dayname = 'habituation day#3',
                                                                  snd.dayname = 'learning2 day#1')),
                comparison='3_beforetest')
)
```

```{r}
pf.cors.df = bind_rows(
  habit.learning.pf.cor,
  filter(run.trials.si, day_desc == 'habituation day#3') %>% 
    dplyr::mutate(session.cor=early.late.cor, comparison='1_within-day')) %>%
  filter(!is.na(session.cor))

pf.cors.summary = group_by(pf.cors.df, comparison, implant) %>%
  dplyr::summarise(cor.mean = mean(session.cor),
                   precentile.05cor = (ecdf(session.cor))(0.5),
                   cor.quant.25 = quantile(session.cor, c(0.75)))

pf.cors.df %>%
  ggplot(aes(x=session.cor, group=comparison, fill=comparison)) +
  geom_vline(xintercept = 0, linetype='dashed', color='#333333') +
  geom_density(aes(y=..scaled..), 
               alpha=0.4) +
  geom_vline(data=pf.cors.summary, mapping=aes(xintercept=cor.mean),
             linetype='dotted') +
  scale_fill_manual(values=rev(three.colours)) +
  facet_grid(. ~ implant + comparison, scales='free_x') +
  coord_flip() +
  ylab('Density') + xlab('Correlation') +
  scale_y_continuous(breaks=c(0, 1)) +
  xlim(c(-1, 1)) +
  gtheme
figure.ggsave('Figure1-field-correlation.pdf', width=8.5, height=4.3)
```

```{r}
pf.cors.df$comparison = as.factor(pf.cors.df$comparison)
m = lmer.test.print(pf.cors.df,
                var = session.cor,
                fixed.effects = implant + comparison,
                diagnostics.groupvar = animal,
                randef.str = '(1 | animal)')

# Effect of comparison
models = create.bayes.lm.pair(pf.cors.df,
                              formula.full = session.cor ~ 1 + animal + implant + comparison,
                              formula.null = session.cor ~ 1 + animal + implant,
                              whichRandom = 'animal')
...
models = create.bayes.lm.pair(pf.cors.df,
                              formula.full = session.cor ~ 1 + animal + implant + comparison,
                              formula.null = session.cor ~ 1 + animal + comparison,
                              whichRandom = 'animal')
models$full / models$null
...
m = lmer.test.print(pf.cors.df,
                ...
                diagnostics.groupvar = animal,
                randef.str = '(1 | animal)')
lsmeans.post.hoc(m, pairwise = TRUE)
lsmeans.post.hoc(m)

models = create.bayes.lm.pair(filter(pf.cors.df, comparison != '1_within_day'),
                              formula.full = session.cor ~ 1 + animal + comparison,
                              formula.null = session.cor ~ 1 + animal ,
                              whichRandom = 'animal')
models$full / models$null
calc.pair.95CI(models$full, pair.vars = c('comparison-2_habit', 'comparison-3_beforetest'),
...
single.day.pcs.df = filter(compared.pcs.df, day_desc=='habituation day#3') %>%
  filter(signif.si)
...
plot.pc.stat = function(metric, facet.exp_title=FALSE) {
  ...
  g = single.day.pcs.df %>%
    ...
    geom_jitter(shape=16, width=0.4, height=0, size=0.11, alpha=0.15, color='#333333')+
    gtheme +
    xlab('') +
...
figure.ggsave('Figure_1DEGH.pdf', width=12.2, height=3.4)
```


```{r}
print('Field size signif larger in vCA1')
field.size.model = lmerTest::lmer(log(field.size.50 ) ~ implant + (1 | animal), 
                                  #log(field.size.50 ) ~ implant + (1 | aday),
...
pc.count.model = lmerTest::lmer(pc.pct ~ implant + (1 | animal), 
                                data=summary.compared.pcs.df, REML=TRUE)
...
figure.ggsave('Figure_2FH_Right.pdf', height=5, width=4.9)
```
...
pc.ids = dplyr::group_by(beforetest.peaks.at.current.rew, implant, animal, cell_id) %>%
  dplyr::summarise(nsignif=sum(signif.si), 
                   n=n()) %>%
  dplyr::filter(n > 1) %>% # only include cells present on at least two days and
  dplyr::filter(nsignif/n >= 0.5) %>% # which were classified as place cells > half the time
...
shuffled.cell.atrew.hist = shuffled.cell.dist.summary %>%
  ...
  dplyr::summarise(create.hist.tibble(rew.peaks.pct, seq(-rew.peaks.pct.step,100,rew.peaks.pct.step)))

# Percentile of cells in the shuffles with few and many fields
shuffled.pct.atrew.95p = shuffled.cell.atrew.hist %>%
  dplyr::mutate(few_fields = upper <= few.fields.thr, many_fields = lower >= many.fields.thr) %>%
...
print('vCA1 Infrequent rewards percentile')
vca1.infreq.rew.fields = max(subset(cell.rew.atrew.hist, implant == 'vCA1' & mid <= 10)$pct.count.cumsum)
mean(subset(shuffled.pct.atrew.95p, implant=='vCA1' & few_fields)$z[[1]] < vca1.infreq.rew.fields)
...
print('vCA1 Infrequent rewards percentile')
dca1.infreq.rew.fields = max(subset(cell.rew.atrew.hist, implant == 'dCA1' & mid <= 10)$pct.count.cumsum)
mean(subset(shuffled.pct.atrew.95p, implant=='dCA1' & few_fields)$z[[1]] < dca1.infreq.rew.fields)
...
figure.ggsave('Figure_S4D.pdf', height=3.3, width=5.2)
```
```{r}
learning.cell.recall = place.cell.db %>%
  #filter(exp != 'habituation') %>%
  group_by(implant, animal, cell_id) %>%
  dplyr::summarise(nrecalled=n(), .groups='drop')

count(learning.cell.recall, implant)
count(learning.cell.recall, implant, animal) %>%
  dplyr::group_by(implant) %>%
  dplyr::summarise(mean(n), sem(n), nanimals=n())

ggplot(learning.cell.recall) +
  geom_histogram(aes(x=nrecalled, color=implant),
                 fill='transparent',
                 binwidth = 1) + 
  facet_grid(implant ~ ., scales='free_y') +
  scale_x_continuous(breaks=seq(3,15,3)) +
  xlab('Days recalled') + ylab('# Cells') +
  scale_color_manual(values=main.two.colours) +
  gtheme +
  theme(legend.position = 'none')
figure.ggsave('Figure_S4C.pdf', height=3.9, width=4.8)
```
