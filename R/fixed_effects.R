...
lmer.test.print = function(df, 
  ...
  fixed_varvals = dplyr::mutate(df, x = !!diagnostics.groupvar) %>% pull(x)
  ...
  if (print.mean.stats) {                                                       
    print('Mean stats')
    df %>%
      dplyr::mutate(effect=!!fixed.effects %>% as.character()) %>%
...
lsmeans.post.hoc = function(m.full, p.adjust.method='holm', ...) {
  x = lsmeansLT(m.full, ddf='Satterthwaite', ...)
  x$`Pr(>|t|).adj` = p.adjust(x$`Pr(>|t|)`, method=p.adjust.method)
  x                                                                             
}
